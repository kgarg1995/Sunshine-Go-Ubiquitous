...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                JSONObject todayTemperatureObject =
                        todayForecast.getJSONObject(OWM_TEMPERATURE);
                double maxTemp = todayTemperatureObject.getDouble(OWM_MAX);
                double minTemp = todayTemperatureObject.getDouble(OWM_MIN);

                JSONObject todayWeatherObject = todayForecast
                        .getJSONArray(OWM_WEATHER).getJSONObject(0);
                int todayWeatherId = todayWeatherObject.getInt(OWM_WEATHER_ID);

                pushToDataLayer(maxTemp, minTemp, todayWeatherId);
                updateWidgets();
                updateMuzei();
                notifyWeather();
            }
    ...
    private void pushToDataLayer(double maxTemp, double minTemp, int weatherId) {
        PutDataMapRequest putDataMapRequest =
                PutDataMapRequest.create("/weather");
        putDataMapRequest.getDataMap().putString("timestamp", String.valueOf(System.currentTimeMillis()));
        putDataMapRequest.getDataMap().putString("maxTemp", Utility.formatTemperature(getContext(), maxTemp));
        putDataMapRequest.getDataMap().putString("minTemp", Utility.formatTemperature(getContext(), minTemp));
    ...
    public void onConnectionSuspended(int i) {
        Log.d(LOG_TAG, String.format("onConnectionSuspended(%d)", i));
    }
